...
public class MeasureEnergyMojo extends AbstractMojo {
    ...
    /**
     * Full path to the Joular Core binary.
     * When left blank the plugin auto-downloads the correct binary for the current
     * platform and caches it in {@code ~/.greener/cache/joularcore/}.
     */
    @Parameter(property = "greener.joularCoreBinaryPath")
    private File joularCoreBinaryPath;

    /**
     * Joular Core version to download when {@link #joularCoreBinaryPath} is not set.
     * See https://github.com/joular/joularcore/releases for available versions.
     */
    @Parameter(property = "greener.joularCoreVersion",
               defaultValue = JoularCoreConfig.DEFAULT_VERSION)
    private String joularCoreVersion;

    /**
     * Hardware component monitored by Joular Core.
     * Valid values: {@code cpu}, {@code gpu}, {@code all}.
     */
     ...
     * Defaults to common Spring Petclinic endpoints when not configured.
     */
    @Parameter
     ...
     * built-in HTTP loader (e.g. {@code k6 run script.js}).
     * The {@code APP_URL} environment variable is set to {@link #baseUrl}.
     */
     ...
     * Takes precedence over {@link #externalTrainingCommand} when set.
     *
     * <p>Available environment variables in the script:
     * {@code APP_URL}, {@code APP_HOST}, {@code APP_PORT},
     * {@code WARMUP_SECONDS}, {@code MEASURE_SECONDS}, {@code TOTAL_SECONDS}, {@code RPS}.
     *
     * <p>Examples: {@code examples/workloads/oha/run.sh},
     * {@code examples/workloads/wrk/run.sh},
     * {@code examples/workloads/gatling/run.sh}
     */
     ...
     *
     * <p>In VM mode Joular Core cannot read RAPL counters directly and instead reads
     * the VM's total power from a file written by the host — see {@link #vmPowerFilePath}.
     * This flag passes {@code --vm} to the joularcore binary.
     *
     * <h2>Host-side setup</h2>
     * The host must write the VM's instantaneous power (Watts) to {@link #vmPowerFilePath}
     * once per second.  A mock script is included for CI pipelines; see
     * {@code .github/workflows/energy-baseline.yml} for an example.
     */
    @Parameter(property = "greener.vmMode", defaultValue = "false")
    private boolean vmMode;

    /**
     * Path to the VM power file that Joular Core reads when {@link #vmMode} is {@code true}.
     * The file must contain a single floating-point number (e.g. {@code 45.2}).
     */
     ...
     * Defaults to {@code ${project.basedir}/energy-baseline.json}.
     */
    @Parameter(property = "greener.baselineFile",
               defaultValue = "${project.basedir}/energy-baseline.json")
    private File baselineFile;

    /**
     * Maximum allowed percentage increase in total energy before the build is failed.
     * For example {@code 10} means a 10 % regression is tolerated.
     */
    ...
    /**
     * Directory where {@code greener-energy-report.html} is written.
     * Defaults to {@code ${project.build.directory}/greener-reports}.
     */
    @Parameter(property = "greener.reportOutputDir",
               defaultValue = "${project.build.directory}/greener-reports")
    ...
    private void executeInternal() throws Exception {
        ...
        try {
            ...
            try {
                ...
                if (warmupDurationSeconds > 0) {
                    ...
                    TrainingConfig warmupConfig = buildTrainingConfig()
                            .warmupDurationSeconds(warmupDurationSeconds)
                            .measureDurationSeconds(0);
                    new TrainingRunner().run(warmupConfig);

                    // Discard warmup readings by deleting the CSV and restarting Joular Core
                ...
                TrainingConfig measureConfig = buildTrainingConfig()
        ...
        EnergyReport report = resultReader.readResults(
                outputCsv, buildRunId(), measureDurationSeconds, appIdentifier);
        ...
        Path htmlReport = new HtmlReporter().generateReport(
                report, comparison, workloadStats, reportOutputDir.toPath());
        getLog().info("HTML report: " + htmlReport);

        // 11. Fail build on regression?
        if (failOnRegression && comparison.isFailed()) {
            throw new MojoFailureException(String.format(
                    "Energy regression detected: %.2f%% increase exceeds threshold of %.1f%%. "
                    + "See report at: %s",
