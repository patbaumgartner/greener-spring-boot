...
/**
 * Promotes the energy report produced by the last {@code greener:measure} execution
 * to become the new baseline.
 *
 * <p>Intended to be run manually (or on the main branch in CI) after a run whose
 * energy consumption you accept as the reference:
 * <pre>
 * mvn greener:update-baseline
 * </pre>
 *
 * <p>The baseline JSON file is written to {@link #baselineFile} and should be
 * committed to source control or cached as a CI artifact so that PR builds can
 * compare against it.
 */
...
public class UpdateBaselineMojo extends AbstractMojo {
     ...
     * Defaults to {@code ${project.build.directory}/greener-reports/work/}.
     *
     * <p>When set, the report at this path is loaded and saved as the new baseline.
     * When not set, the mojo reads the most recent report from the default work directory.
     */
    ...
    @Parameter(property = "greener.commitSha",
               defaultValue = "${env.GITHUB_SHA}")
    private String commitSha;

    /** Branch name recorded in the baseline (e.g. from {@code GITHUB_REF_NAME}). */
    @Parameter(property = "greener.branch",
               defaultValue = "${env.GITHUB_REF_NAME}")
    ...
    public void execute() throws MojoExecutionException {
        ...
        try {
            ...
            if (existing.isEmpty()) {
                getLog().warn("No existing energy report to promote to baseline at: "
                        + baselineFile
            ...
            manager.saveBaseline(report, normalise(commitSha), normalise(branch),
                    baselineFile.toPath());
