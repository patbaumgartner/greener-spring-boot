 ...
 * across runs are comparable. Two modes are supported:
 * <ul>
 * <li><b>Built-in HTTP loader</b> — sends configurable GET requests using the Java
 * {@link java.net.http.HttpClient} at the configured rate.</li>
 * <li><b>External command</b> — delegates to an external process (e.g. k6, wrk, a shell
 * script). The {@code APP_URL} environment variable is set to the application base
 * URL.</li>
 * </ul>
 */
public class TrainingConfig {
	...
	/**
	 * Relative URL paths to GET during the training run. Configure these to match the key
	 * endpoints of your application (e.g. add {@code /owners}, {@code /vets} for Spring
	 * Petclinic).
	 */
	...
	/**
	 * Optional external command to use as the training workload instead of the built-in
	 * HTTP loader. The application base URL is available as {@code APP_URL}, warmup
	 * seconds as {@code WARMUP_SECONDS}, measure seconds as {@code MEASURE_SECONDS}.
	 *
	 * <p>
