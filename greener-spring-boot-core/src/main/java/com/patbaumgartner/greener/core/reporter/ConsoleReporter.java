...
public class ConsoleReporter {
    ...
    public void report(EnergyReport current, ComparisonResult comparison,
        ...
        if (!current.measurements().isEmpty()) {
            ...
            current.topMethods(topN).forEach(m ->
                    System.out.printf("   %-55s  %10.4f%n",
                            truncate(m.methodName(), 55), m.energyJoules()));
        } else {
    ...
    private void printWorkloadStats(WorkloadStats stats, double totalEnergyJoules) {
        ...
        if (stats.hasRequestCounts()) {
            System.out.printf("   Requests     : %d total, %d failed (%.1f%%)%n",
                    stats.totalRequests(), Math.max(0, stats.failedRequests()),
        ...
        if (!Double.isNaN(mjPerReq)) {
            ...
            System.out.printf("   Avg Power    : %.2f W  (energy/req unavailable — no request count)%n",
                    wattsAvg);
        }
    }
    ...
    private void printComparison(ComparisonResult comparison) {
        ...
        List<MethodComparison> regressions = comparison.methodComparisons().stream()
        ...
        if (!regressions.isEmpty()) {
            ...
            regressions.forEach(mc ->
                    System.out.printf("   %-48s  %8.4f  %8.4f  %+8.2f%n",
                            truncate(mc.methodName(), 48),
                            mc.baselineEnergyJoules(),
                            mc.currentEnergyJoules(),
                            mc.deltaPercent()));
        }
