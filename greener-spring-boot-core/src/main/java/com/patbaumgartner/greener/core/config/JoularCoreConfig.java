 ...
 *
 * <h2>How Joular Core is used</h2>
 * Joular Core is a native Rust binary that reads hardware power counters (RAPL on Linux/Windows,
 * {@code powermetrics} on macOS) and can monitor a specific PID or application name.
 * The plugin starts Joular Core alongside the Spring Boot application, lets it record power
 * samples to a CSV file, and then computes energy = Σ(power_watts × sample_interval_s).
 *
...
public class JoularCoreConfig {
     ...
     * Set at runtime after the application process has started.
     */
    private Long pid;

    /**
     * Monitor all processes matching this application name (alternative to PID).
     * When both {@link #pid} and {@link #appName} are set, PID takes precedence.
     */
    ...
    /**
     * Suppress terminal output from Joular Core (pass the {@code -s} flag).
     * Recommended to keep plugin output readable.
     */
    private boolean silent = true;

    /**
     * Whether to use Joular Core also as the power backend for JoularJX (when JoularJX
     * is enabled for method-level monitoring). Maps to {@code joular-core=true} in
     * JoularJX {@code config.properties}.
     */
    private boolean useWithJoularJx = false;

    /**
     * Path to the Joular Core shared-memory ring buffer file.
     * Only relevant when {@link #useWithJoularJx} is {@code true} and
     * JoularJX is configured with {@code joular-core-ring-buffer=true}.
     */
     ...
     *
     * <p>In VM mode Joular Core cannot read hardware power counters directly
     ...
     * {@link #vmPowerFilePath} every second.  The recommended approach is
     * <a href="https://github.com/hubblo-org/scaphandre">Scaphandre</a> running
     * on the KVM host.  See {@code examples/vm-setup/README.md} for the full
     * setup guide covering virtio-mem, file-export, and RAPL MSR passthrough.
     */
    ...
    /**
     * Extra CLI arguments appended verbatim to the joularcore invocation.
     * Useful for flags not yet covered by explicit properties.
     */
    ...
    public Path getOutputCsvPath() { return outputCsvPath; }
    public boolean isSilent() { return silent; }
    public boolean isUseWithJoularJx() { return useWithJoularJx; }
    public Path getRingBufferPath() { return ringBufferPath; }
