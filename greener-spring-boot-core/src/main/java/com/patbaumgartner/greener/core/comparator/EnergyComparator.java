 ...
 *   <li>{@code delta > +threshold} → {@link ComparisonStatus#REGRESSED} (build may fail).</li>
...
public class EnergyComparator {
     ...
     */
    public ComparisonResult compare(EnergyReport current,
                                    Optional<EnergyBaseline> baseline,
                                    double threshold) {
        if (baseline.isEmpty()) {
            return new ComparisonResult(
                    ComparisonStatus.NO_BASELINE,
                    0, current.totalEnergyJoules(), 0,
                    List.of(), false, threshold);
        }
        ...
        return new ComparisonResult(status, baselineTotal, currentTotal,
                totalDelta, methodComparisons, thresholdBreached, threshold);
    }

    private List<MethodComparison> buildMethodComparisons(EnergyReport current,
                                                          EnergyReport baseline) {
        Map<String, Double> baselineByMethod = baseline.measurements().stream()
                .collect(Collectors.toMap(EnergyMeasurement::methodName,
                        EnergyMeasurement::energyJoules, Double::sum));
        ...
        for (EnergyMeasurement m : current.measurements()) {
            ...
            comparisons.add(new MethodComparison(m.methodName(), baselineValue,
                    m.energyJoules(), delta));
        }

        // Methods present in baseline but absent from current run
        for (Map.Entry<String, Double> entry : baselineByMethod.entrySet()) {
            boolean inCurrent = current.measurements().stream()
                    .anyMatch(m -> m.methodName().equals(entry.getKey()));
            if (!inCurrent) {
