 ...
 *
 * <p>
 * This data is used to compute energy-per-request metrics that make the energy report
 * meaningful: instead of reporting raw Joules, the report can show how much energy a
 * single business transaction costs.
 *
 * <h2>Tool names</h2>
 * <ul>
 ...
 *
 * <p>
 * {@code totalRequests} and {@code failedRequests} are {@code -1} when unknown (i.e. the
 * external tool did not report these figures).
 */
public record WorkloadStats(String tool, long totalRequests, long failedRequests, long durationSeconds) {
	...
	/**
	 * Requests per second (throughput) during the measurement window. Returns
	 * {@link Double#NaN} when counts or duration are unavailable.
	 */
	public double requestsPerSecond() {
		if (!hasRequestCounts() || durationSeconds <= 0)
			return Double.NaN;
	 ...
	 *
	 * <p>
	 * A lower value indicates a more energy-efficient code path. Returns
	 * {@link Double#NaN} when request counts are unavailable or zero.
	...
	public double energyPerRequestMillijoules(double totalEnergyJoules) {
		long successfulRequests = hasRequestCounts() ? totalRequests - Math.max(0, failedRequests) : UNKNOWN;
	...
	/**
	 * Failure rate as a percentage of total requests. Returns {@link Double#NaN} when
	 * counts are unavailable or total is zero.
	 */
	public double failureRatePercent() {
		if (!hasRequestCounts() || totalRequests <= 0)
			return Double.NaN;
